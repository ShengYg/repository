...
适用场景：向多个对象发出请求，并且不知道该哪个对象处理时，将这些对象**连成一条链**，并沿着这条链传递该请求，直到有一个对象处理它为止。构造函数中，利用指针使派生类对象连成链。
...
<a name='observer'></a>
## 观察者模式 observer
适用场景：定义对象间的一种**一对多的依赖关系**，当一个对象的状态发生改变时，所有依赖于它的对象都得到通知并被自动更新。

例子：举个博客订阅的例子，当博主发表新文章的时候，即博主状态发生了改变，那些订阅的读者就会收到通知，然后进行相应的动作，比如去看文章，或者收藏起来。可以建一个`Blog`类，里面存储`Observer`的链表，调用`Observer.update()`来通知对方。
